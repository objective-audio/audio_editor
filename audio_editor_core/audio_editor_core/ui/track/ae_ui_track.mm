...
ui_track::ui_track(std::shared_ptr<ui::standard> const &standard, std::shared_ptr<track_presenter> const &presenter,
    ...
    : _presenter(presenter),
      ...
      _modules(ui_modules::make_shared(ui_track_constants::max_plane_count)),
      _markers(markers) {
    ...
    presenter->observe_modules([this](auto const &elements) { this->_modules->set_elements(elements); })
        .sync()
        ->add_to(this->_pool);
...
std::shared_ptr<ui_track> ui_track::make_shared(std::shared_ptr<ui::standard> const &standard,
    ...
    return std::shared_ptr<ui_track>(new ui_track{standard, presenter, markers});
}
