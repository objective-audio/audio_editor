...
std::shared_ptr<import_interactor> import_interactor::make_shared(project_sub_level_router *router,
                                                                  editing_status const *editing_status,
                                                                  file_module_loader *file_module_loader) {
    return std::make_shared<import_interactor>(router, editing_status, file_module_loader);
}

import_interactor::import_interactor(project_sub_level_router *router, editing_status const *editing_status,
                                     file_module_loader *file_module_loader)
    : _router(router), _editing_status(editing_status), _file_module_loader(file_module_loader) {
}

bool import_interactor::can_select_file_for_import() const {
    return this->can_import_from_file() && !this->_router->sub_level().has_value();
}
...
void import_interactor::select_file_for_import() {
    ...
    this->_router->add_dialog(dialog_content::select_file_for_import);
}
