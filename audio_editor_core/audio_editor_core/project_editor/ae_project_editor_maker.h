...
struct project_editor_maker final : project_editor_maker_interface {
    ...
    static std::shared_ptr<project_editor_maker> make_shared(std::shared_ptr<timeline_player_interface> const &);

   private:
    std::shared_ptr<timeline_player_interface> const _player;

    project_editor_maker(std::shared_ptr<timeline_player_interface> const &);
