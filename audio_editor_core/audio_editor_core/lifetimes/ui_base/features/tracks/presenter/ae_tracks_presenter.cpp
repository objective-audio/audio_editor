...
std::shared_ptr<tracks_presenter> tracks_presenter::make_shared(project_lifetime_id const &project_lifetime_id) {
    ...
    return std::make_shared<tracks_presenter>(project_lifetime_id, project_editing_lifetime->track_content_pool,
                                              project_editing_lifetime->selected_track_pool,
                                              project_lifetime->display_space,
                                              project_editing_lifetime->display_space_range);
}
...
tracks_presenter::tracks_presenter(project_lifetime_id const &project_lifetime_id,
    ...
    : _content_pool(content_pool), _selected_pool(selected_pool), _display_space_range(display_space_range) {
    display_space
...
void tracks_presenter::_update_all_contents(bool const force_updating, bool const force_replacing) {
    ...
    if (space_range.has_value() && fulfilled(locked)) {
        ...
        while (yas_each_next(each)) {
            auto const &identifier = yas_each_index(each);
            contents.emplace_back(
                track_content{.identifier = identifier, .is_selected = selected_pool->contains(identifier)});
        }
