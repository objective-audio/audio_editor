...
module::module(std::string const &name, time::range const &range, frame_index_t const file_frame,
...
module module::offset(frame_index_t const offset) const {
    return module{this->name, this->range.offset(offset), this->file_frame, this->file_name};
}
...
std::string yas::to_string(ae::module const &module) {
    return std::string{"{file_frame:" + std::to_string(module.file_frame) + ", range:" + to_string(module.range) +
                       ", name:" + module.name + ", file_name:" + module.file_name + "}"};
}
