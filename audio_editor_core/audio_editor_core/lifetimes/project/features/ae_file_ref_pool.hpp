...
struct file_ref_pool final {
    ...
    void revert(std::vector<file_ref> &&);
    void insert(file_ref::params const &);

    [[nodiscard]] std::optional<file_ref> ref(std::string const &) const;

   private:
    database_for_file_ref_pool *_database;

    observing::map::holder_ptr<std::string, file_ref> const _refs;
