...
marker_location::marker_location(object_id const &identifier, frame_index_t const &frame, uint32_t const sample_rate,
                                 float const scale)
    : marker_location(identifier, marker_location_utils::to_x(frame, sample_rate, scale)) {
}

marker_location::marker_location(object_id const &identifier, float const x) : identifier(identifier), x(x) {
}
