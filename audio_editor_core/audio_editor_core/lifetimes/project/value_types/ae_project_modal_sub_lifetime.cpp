...
template std::shared_ptr<sheet_lifetime> const &yas::get(std::optional<ae::project_modal_sub_lifetime> const &);
...
ae::project_modal_sub_lifetime_kind yas::to_kind(std::optional<ae::project_modal_sub_lifetime> const &sub_lifetime) {
    ...
    } else if (std::holds_alternative<std::shared_ptr<sheet_lifetime>>(sub_lifetime.value())) {
        return kind::sheet;
