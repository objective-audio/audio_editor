...
std::shared_ptr<modules_presenter> modules_presenter::make_shared(window_lifetime_id const &window_lifetime_id,
    ...
    return std::make_shared<modules_presenter>(project_lifetime->project_format, window_lifetime->player,
                                               ...
                                               project_lifetime->module_content_pool);
}
        ...
        ->observe_event([this, sample_rate](file_track_event const &event) {
            ...
            switch (event.type) {
                ...
                case file_track_event_type::erased:
                    content_pool->erase_for_id(event.module.value().identifier);
                    break;
                case file_track_event_type::inserted: {
                    ...
                    if (space_range.has_value() && module.value.range.is_overlap(space_range.value())) {
                        content_pool->insert({module, sample_rate, space_range.value(), display_space->scale().width});
                    }
                } break;
                case file_track_event_type::detail_updated: {
                    ...
                    auto const space_range = this->_space_range();
                    if (space_range.has_value() && module.value.range.is_overlap(space_range.value())) {
                        content_pool->replace({module, sample_rate, space_range.value(), display_space->scale().width});
                    }
                } break;
            }
        })
...
void modules_presenter::_update_all_contents(bool const force_updating, bool const force_replacing) {
    ...
    if (player && file_track && space_range.has_value() && display_space && content_pool) {
        auto const current_frame = player->current_frame();
        ...
        auto const contents = filter_map<module_content>(
            ...
            [&space_range_value, sample_rate = this->_project_format.sample_rate, &scale](auto const &module) {
                if (module.first.range.is_overlap(space_range_value)) {
                    return std::make_optional<module_content>(module.second, sample_rate, space_range_value, scale);
                } else {
