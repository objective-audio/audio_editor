...
std::shared_ptr<app_presenter> app_presenter::make_shared() {
    return std::make_shared<app_presenter>(hierarchy::app_lifetime()->project_lifecycle,
                                           hierarchy::app_lifetime()->dialog_lifecycle,
                                           hierarchy::app_lifetime()->project_preparer);
}

app_presenter::app_presenter(std::shared_ptr<project_lifecycle_for_app_presenter> const &pool,
                             std::shared_ptr<app_dialog_lifecycle> const &dialog_lifecycle,
                             std::shared_ptr<project_preparer> const &preparer)
    : _project_lifecycle(pool), _dialog_lifecycle(dialog_lifecycle), _project_preparer(preparer) {
}
...
observing::syncable app_presenter::observe_window(std::function<void(app_presenter_window_event const &)> &&handler) {
    if (auto const lifecycle = this->_project_lifecycle.lock()) {
        return lifecycle->observe_event([handler = std::move(handler)](project_lifecycle_event const &event) {
