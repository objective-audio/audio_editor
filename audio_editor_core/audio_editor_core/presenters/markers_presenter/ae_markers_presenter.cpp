...
std::shared_ptr<markers_presenter> markers_presenter::make_shared(
    ...
    std::shared_ptr<display_space> const &display_space) {
    return std::shared_ptr<markers_presenter>(new markers_presenter{editor, display_space});
}

markers_presenter::markers_presenter(std::shared_ptr<project_editor_for_markers_presenter> const &editor,
                                     std::shared_ptr<display_space> const &display_space)
    : _project_editor(editor), _display_space(display_space), _location_pool(marker_location_pool::make_shared()) {
...
void markers_presenter::_update_all_locations(update_type const type) {
    ...
    if (editor && space_range.has_value()) {
        ...
        auto const locations = filter_map<marker_location>(
            editor->markers(), [&space_range_value, sample_rate = editor->file_info().sample_rate,
