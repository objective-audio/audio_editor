...
std::shared_ptr<editing_root_presenter> editing_root_presenter::make_shared(
    ...
    return std::make_shared<editing_root_presenter>(window_lifetime_id, window_lifetime->player,
                                                    app_lifetime->action_sender);
}

editing_root_presenter::editing_root_presenter(window_lifetime_id const &window_lifetime_id,
                                               std::shared_ptr<player> const &player,
                                               std::shared_ptr<action_sender> const &action_sender)
    : _window_lifetime_id(window_lifetime_id), _player(player), _action_sender(action_sender) {
}
