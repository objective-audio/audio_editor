...
#include <ae-core/project_editing/value_types/ae_module_utils.h>

using namespace yas;
using namespace yas::ae;

module::module(std::string const &name, time::range const &range, track_index_t const track,
               frame_index_t const file_frame, std::string const &file_name) :name(name),
    range(range), track(track), file_frame(file_frame), file_name(file_name) {
}
